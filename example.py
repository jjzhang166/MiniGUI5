...
def run_mode1():
    ...
    canvas.draw_line("1.Welcome page", [int(canvas.width * 0.3), 3])
    canvas.draw_line("2.print any upper case on screen", [int(canvas.width * 0.3), 4])
    canvas.draw_line("3.let it be dynamic", [int(canvas.width * 0.3), 5])
    canvas.draw_line("Please enter 0-3 to select", [int(canvas.width * 0.3), 7])
    canvas.draw_text("WELCOME", [int(canvas.width * 0.15), 10])
    canvas.update()
    change_mode()
